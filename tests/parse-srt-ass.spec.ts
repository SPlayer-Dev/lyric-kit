...
只有一行原词`;
    ...
    const { lines, metadata } = parseSRT(srt);
...
Dialogue: 0,0:00:01.00,0:00:03.00,roma,,0,0,0,,Ni Hao`;
    ...
    const { lines, metadata } = parseASS(ass);
  ...
  it("应支持提取 ASS 元数据与识别 AMLL 导出的声部", () => {
...
[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
Dialogue: 0,0:00:01.00,0:00:03.00,v2,,0,0,0,,{\\kf200}对唱主词
Dialogue: 0,0:00:01.00,0:00:03.00,v2-bg,,0,0,0,,{\\kf200}和声`;

    const { lines, metadata } = parseASS(ass, { extractMetadata: true });

    expect(metadata.title).toEqual(["七里香"]);
    expect(metadata.authors).toEqual(["周杰伦"]);
    expect(lines).toHaveLength(2);
